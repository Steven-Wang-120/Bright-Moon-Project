...
这个目录是一套面向 `STM32H743ZIP6` 的最小自包含固件工程，目标是直接接收香橙派串口发来的一行 JSON，然后在 SSD1306 屏幕上显示表情。
...
我选的是比较通用、在 `STM32H743ZI` 封装上可用的一组引脚：
...
如果你板子上这组脚不方便，可以直接改 [inc/board.h](inc/board.h)。
...
```text
READY stm32_h743_uart_face_display v0.1.0 json-line stm32h743 uart1 pa9-pa10 oled pb8-pb9
```

## 表情显示

为了保持工程自包含，这一版没有依赖外部素材文件，而是直接在 SSD1306 上画 6 种基础表情：
...
这样最适合当前“先打通串口收 JSON 控屏”的最小可用目标。
...
- [src/ssd1306.c](src/ssd1306.c)：软件 I2C 和 SSD1306 刷屏
